...
<head>
    <title>E-Scoot | Pregled Rezervacija</title>
    <meta charset="utf-8">
...
<body class="hide">
    ...
    <section class="caption" style="margin-top:100px">
        <br>
        <h1><span style="font-size:xxx-large; font-weight: 100; color: #ededed; filter: drop-shadow(0px 0px 1px #000);">E-Scoot </span><span style="text-align: center; font-size:xxx-large; color: #49c5b6; filter: drop-shadow(0px 0px 15px #49C5B6);"> RAČUN</span></h1>
    </section>
    ...
    ?>
            <h2 class="bookdet"><?php echo 'ID rezervacije: #' . $bookid ?></h2>
            <h2 class="bookdet"><?php echo 'Datum izdavanja računa: ' . $billdate  ?></h2>
            <div class="cal">
                <table class="invc">
                    <tr>
                        <th>Model</th>
        ...
        $view3 = "SELECT DISTINCT * FROM billing_details WHERE DL_NUM='$dlnum'";
        $v3 = $conn->query($view3);
        ?><div class="wrapper">
            <div>
                <form method="post">
                    <label class="booking" for="book-id">Unesite svoj ID rezervacije za otkazivanje: </label><br>
                    
                    <select name="cancel-id" required>
                        <option> Odaberite ID rezervacije </option>
                        <?php
                    ...
                    <br>
                    <input class="cancbut" id="cancel" type="submit" name='cancel' value="OTKAŽI">
                </form>
            </div>
            ...
            <button style="height: 40px; width: 120px; color: #ededed; background: #49c5b6;" class="but"><a href="status.php?id=<?php echo $_SESSION['email'] ?>"></a>OSVJEŽI</button>

        </div>
    <?php
    } else { ?>
        <h1 class="red-font" style="font-size:30px; margin:60px;">NEMATE REZERVACIJA!</h1>
    <?php
    }
    ?>
